...
struct TransmissionControlProtocolHeader {
  ...
  // we also have variable sized options, and data
...
public:
  ...
  virtual bool HandleTrasmissionControlMessage(common::uint8_t *data,
                                               common::uint16_t size);
...
public:
  TransmissionControlProtocolProvider(InternetProtocolProvider *provider);
