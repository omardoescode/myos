...
void UserDatagramProtocolHandler::HandlerUserDataProtocolMessage(
    UserDatagramProtocolSocket *socket, common::uint8_t *data,
...
void UserDatagramProtocolSocket::HandlerUserDataProtocolMessage(
    common::uint8_t *data, common::uint16_t size) {
  if (handler != 0)
    handler->HandlerUserDataProtocolMessage(this, data, size);
}

void UserDatagramProtocolSocket::Send(common::uint8_t *data,
                                      common::uint16_t size) {
  backend->Send(this, data, size);
}
void UserDatagramProtocolSocket::Disconnect() { backend->Disconnect(this); }

// PROVIDER

UserDatagramProtocolProvider::UserDatagramProtocolProvider(
    InternetProtocolProvider *provider)
    : InternetProtocolHandler(provider, 0x11), numSockets(0), freePort(1024) {
...
bool UserDatagramProtocolProvider::onInternetProtocolReceived(
  ...
  UserDatagramProtocolHeader *msg =
      (UserDatagramProtocolHeader *)internetProtocolPayload;
  ...
  for (int i = 0; i < numSockets && socket == 0; i++) {
    if (sockets[i]->localPort == msg->dstPort &&
        sockets[i]->localIP == dstIP_BE && sockets[i]->listening) {
      socket = sockets[i];
  ...
  if (socket != 0)
    socket->HandlerUserDataProtocolMessage(
...
UserDatagramProtocolProvider::Connect(common::uint32_t ip,
...
UserDatagramProtocolSocket *
UserDatagramProtocolProvider::Listen(common::uint16_t port) {
  UserDatagramProtocolSocket *socket =
      (UserDatagramProtocolSocket *)MemoryManager::activeMemoryManager->malloc(
          sizeof(UserDatagramProtocolSocket));
  if (socket != 0) {
...
void UserDatagramProtocolProvider::Disconnect(
    UserDatagramProtocolSocket *socket) {
...
void UserDatagramProtocolProvider::Send(UserDatagramProtocolSocket *socket,
                                        ...
                                        common::uint16_t size) {
  ::printf("UDP send\n");
  common::uint32_t totalLength = size + sizeof(UserDatagramProtocolHeader);
  common::uint8_t *buffer =
      (common::uint8_t *)MemoryManager::activeMemoryManager->malloc(
          totalLength);

  common::uint8_t *buffer2 = buffer + sizeof(UserDatagramProtocolHeader);
  ...
  msg->checksum = 0; // this deactives the checking
