...
void InterruptManager::SetInterruptDescriptorTableEntry(
  ...
  interruptDescriptorTable[interruptNumber].gdt_codeSegment =
      gdt_codeSegmentSelectorOffset;
...
InterruptManager::InterruptManager(uint16_t hardwareInterruptOffset,
    ...
    : picMasterCommand(0x20), picMasterData(0x21), picSlaveCommand(0xA0),
      picSlaveData(0xA1), taskMgr(taskMgr) {

  this->hardwareInterruptOffset = hardwareInterruptOffset;
  uint16_t CodeSegment = gdt->CodeSegmentSelector();
  const uint8_t IDT_INTERRUPT_GATE = 0xE;

  // We will use the ignore request by default to avoid crashes. We will set the
  // others explicitly later
  ...
  hardwarecommunication::Port8BitSlow elcrSlave(0x4D1);
  elcrSlave.Write(0x08);
...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interruptNumber,
  ...
  if (interruptNumber == hardwareInterruptOffset) {
    esp = (common::uint32_t)taskMgr->Schedule((CPUState *)esp);
  }

  if (hardwareInterruptOffset <= interruptNumber &&
      interruptNumber < hardwareInterruptOffset + 16) {
